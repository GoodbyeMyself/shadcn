            ...
            <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
              <div>
                ...
                <div className='flex gap-1'>
                  ...
                  >
                    <p>
                      {t('userManagement.learnMoreSameAsPrefix')}{' '}
                      <Link
                        to='/users'
                    ...
                    <p className='mt-4'>
                      {t('userManagement.learnMoreManageAccount')}
  ...
  return (
    <div className='h-svh'>
      <div className='m-auto flex h-full w-full flex-col items-center justify-center gap-2'>
        ...
        <span className='font-medium'>{t('unauthorized.title')}</span>
        <p className='text-center text-muted-foreground'>
          {t('unauthorized.descriptionPrefix')}{' '}
          <sup>
            <LearnMore open={opened} onOpenChange={setOpened}>
              <p>
                {t('unauthorized.learnMoreSameAsPrefix')}{' '}
                <Link
                  to='/users'
                  className='text-blue-500 underline decoration-dashed underline-offset-2'
                >
              ...
              <p>{t('unauthorized.learnMoreSignIn')}</p>

              <p className='mt-4'>{t('unauthorized.learnMoreAfterSignIn')}</p>
            </LearnMore>
          </sup>
          <br />
          {t('unauthorized.descriptionSuffix')}
        </p>
        <div className='mt-6 flex gap-4'>
          ...
          <Button onClick={() => navigate({ to: '/clerk/sign-in' })}>
            <ClerkLogo className='invert' /> {t('unauthorized.signIn')}
          </Button>
        </div>
        <div className='mt-4 h-8 text-center'>
          {!cancelled && !opened && (
            <>
              <p>
                {countdown > 0
                  ? t('unauthorized.redirectingWithCountdown', { countdown })
                  : t('unauthorized.redirecting')}
              </p>
              <Button variant='link' onClick={() => setCancelled(true)}>
                {t('unauthorized.cancelRedirect')}
              </Button>
            </>
          )}
        </div>
