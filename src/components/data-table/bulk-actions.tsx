...
type DataTableBulkActionsProps<TData> = {
  ...
  getEntityLabel?: (count: number) => string
  labels?: {
    clearSelection?: string
    clearSelectionWithShortcut?: string
    toolbarLabel?: (count: number, entityLabel: string) => string
    announcement?: (count: number, entityLabel: string) => string
    summary?: (entityLabel: string) => string
  }
}
...
export function DataTableBulkActions<TData>({
  ...
  useEffect(() => {
    if (selectedCount > 0) {
      const message =
        labels?.announcement?.(selectedCount, entityLabel) ??
        t('dataTable.bulkActionsAnnouncement', {
          count: selectedCount,
          entity: entityLabel,
        })
  ...
  }, [entityLabel, labels, selectedCount])
  ...
  return (
    <>
      ...
      <div
        ...
        role='toolbar'
        aria-label={
          labels?.toolbarLabel?.(selectedCount, entityLabel) ??
          t('dataTable.bulkActionsToolbar', {
            count: selectedCount,
            entity: entityLabel,
          })
        }
        ...
        >
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={
                  labels?.clearSelection ?? t('dataTable.clearSelection')
                }
                title={
                  labels?.clearSelectionWithShortcut ??
                  t('dataTable.clearSelectionWithShortcut')
                }
              >
                <X />
          ...
          >
            ...
            <span className='hidden sm:inline'>
              {labels?.summary?.(entityLabel) ??
                t('dataTable.summary', { entity: entityLabel })}
            </span>
          </div>
