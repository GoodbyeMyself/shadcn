...
export function TasksImportDialog({
  ...
  const formSchema = z.object({
    file: z
      ...
      .refine((files) => files.length > 0, {
        message: t('dialogs.import.validation.fileRequired'),
      })
      .refine(
        (files) => ['text/csv'].includes(files?.[0]?.type),
        t('dialogs.import.validation.fileFormat')
      ),
  })
  ...
  const onSubmit = () => {
    ...
    if (file && file[0]) {
      ...
      showSubmittedData(fileDetails, t('dialogs.import.submittedTitle'))
    }
    onOpenChange(false)
  }
            ...
            <FormField
              ...
              render={() => (
                <FormItem className='my-2'>
                  <FormLabel>{t('dialogs.import.file')}</FormLabel>
                  <FormControl>
